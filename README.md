...
完成 Phase 4 试改且达到参考条件后，“启用自动模式”按钮才会解锁。点击按钮还必须再次确认真实连续提交；软件不会自行切换模式，也不能通过取消试改选项绕过门槛。

自动模式默认每连续处理 100 份后结束本批并提示检查，教师点击开始可继续下一批。上限可在 1～10000 之间修改；“无限制”只取消批次数量上限，不会取消任何安全暂停条件。`need_review`、三次网络请求均失败、非法 JSON/分数、页面未可靠切换、F8 或紧急停止都会阻止后续自动提交。

每份结果写入用户数据目录的 `trials.db`，包括任务 ID、序号、时间、Provider/模型、请求耗时、完整结构化结果、复核状态、填分/点击提交/页面确认状态、错误以及评分规则版本。正常试卷不长期保存截图；复核和异常截图写入 `review_screenshots` 或 `exception_screenshots`。

自动阅卷暂停后可点击“编辑任务”。软件会先终止持有旧规则的运行；如果题目、满分、步长或评分规则发生变化而用户未手动升版，规则版本会自动加一，只对重新启动后的试卷生效。

自动化测试已模拟连续成功处理 100 份，并验证单次 API 错误立即结束当前批次、不触碰后续试卷。真实 Windows 环境按 [Phase 5 验收清单](PHASE5_CHECKLIST.md) 执行。

## Phase 4：试改系统

启动界面、打开任务并完成坐标标定后，保持“试改模式”勾选，点击“开始自动化测试（真实提交）”。试改默认目标为 100 份，仍会真实填写并向智学网提交分数，必须由教师坐在电脑旁监控。

界面实时显示：

- 已处理数与目标数
- AI 直接正常、AI 主动要求复核、复核后 AI 正确、AI 真实错判
- 漏报错判数 / 自动提交数及漏报错判率
- 是否达到用户设置的自动模式参考阈值

AI 设置 `need_review` 时，本份截图会自动归档且不会提交。教师可标记“复核后 AI 正确”；发现任意错判时可填写正确分数、选择字迹识别/评分标准/物理语义/其他原因并备注，当前截图会归档，正在运行的自动化会安全停止。点击“导出报告”可生成 JSON，包含验收所需总数、AI 正常、AI 主动复核、AI 错判、原因分布与漏报错判率。

数据库和复核截图保存在用户数据目录，不写入任务目录：Windows 为 `%LOCALAPPDATA%\\AIGrader`，macOS 为 `~/Library/Application Support/AIGrader`，Linux 为 `$XDG_DATA_HOME/AIGrader` 或 `~/.local/share/AIGrader`。截图含学生答案，应按学校隐私要求保管和清理。

“已达到自动模式参考条件”仅表示完成目标份数且真实错判率不高于阈值。Phase 4 不提供或自动启用无人值守模式。

100 份模拟流程已纳入自动化测试，统计样例覆盖 70 份 AI 直接正常、20 份 AI 主动复核、15 份 AI 真实错判（其中 10 份漏报），并验证报告内容和自动模式仍为关闭状态。真实 Windows 智学网页面请按 [Phase 4 验收清单](PHASE4_CHECKLIST.md) 实测。
...
测试覆盖任务结构、评分项合计、Prompt、Provider 请求、结构化结果、评分项映射、分数范围/步长、逐空合计、复核原因、Dry Run 错误隔离、自动化安全门，以及 Phase 4 的 100 份试改统计、错判归档和报告导出。真实 API 准确率、Windows 屏幕和智学网页面属于手工验收项。
